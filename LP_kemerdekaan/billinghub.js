...
/** Biaya tetap (Rp) dari API — hanya jika customer_cost angka murni (mis. VA "2000") */
function methodFeeAmount(m) {
  if (m == null) return 0;
  if (typeof m.customer_cost === "number") {
    return Math.max(0, Math.round(m.customer_cost));
  }
  const raw = methodCustomerCostRaw(m);
  if (raw && /^\d+$/.test(raw)) {
    const n = parseInt(raw, 10);
    return isNaN(n) ? 0 : n;
  }
  const fc = m.fee_customer;
  if (fc && typeof fc.flat === "number") return Math.max(0, Math.round(fc.flat));
  const flat = m.fee_flat || 0;
  const n2 = parseInt(String(flat), 10);
  return isNaN(n2) ? 0 : n2;
}

/** Label biaya non-angka dari API (mis. "SPLIT 50:50" untuk QRIS) */
function methodCustomerCostLabel(m) {
  const raw = methodCustomerCostRaw(m);
  if (!raw || /^\d+$/.test(raw)) return "";
  const upper = raw.toUpperCase();
  if (upper.includes("SPLIT") && upper.includes("50")) {
    return "Biaya dibagi 50:50";
  }
  return raw;
}

function pickOrderNumericField(data, keys) {
...
/** HTML baris biaya di kartu metode bayar Qrin — dari customer_cost API */
function buildQrinMethodFeeLineHtml(m) {
  const fee = methodFeeAmount(m);
  if (fee > 0) {
    return `<div class="tripay-method-fee">+Rp ${fee.toLocaleString("id-ID")}</div>`;
  }
  const label = methodCustomerCostLabel(m);
  if (label) {
    return `<div class="tripay-method-fee tripay-method-fee--qris">${escapeHtml(label)}</div>`;
  }
  return "";
}
...
function setQrinOrderFeeSummaryFromPurchase(data) {
  ...
  } else if (adminRp === 0 && totalRp > nominal) {
    adminRp = totalRp - nominal;
  }

  const costLabel = method ? methodCustomerCostLabel(method) : "";
...
function buildQrinOrderSummaryHtml(data) {
  ...
  if (hasAmt) {
    ...
    } else if (fs && fs.costLabel) {
      html += `<div class="qrin-pay-summary-row"><span class="qrin-pay-summary-label">Biaya</span><span class="qrin-pay-summary-value" style="text-align:right;font-size:0.8rem;font-weight:600;max-width:58%">${escapeHtml(fs.costLabel)}</span></div>`;
    }
      ...
      (voucher) => `
        <div class="price-card" onclick="showBuyModal(${voucher.id})">
            ...
            <div class="price-right">
                <span class="price-amount">${voucher.harga}</span>
                <span class="price-cta">Beli paket</span>
            </div>
        </div>
    `
    )
  ...
  try {
    ...
    tripayMethodGrid.innerHTML = methods
      .map((m, idx) => {
        ...
        /* Qrin: biaya dari customer_cost API (angka atau SPLIT 50:50, dll.) */
        const feeLine = buildQrinMethodFeeLineHtml(m);
  ...
  try {
    ...
    if (needPaymentMethod && selectedMethodCode) {
      ...
      if (isQrinPaymentGateway()) {
        ...
        /* Biaya tetap di payload hanya jika customer_cost angka (VA); QRIS SPLIT dihitung backend */
